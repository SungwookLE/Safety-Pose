...
# ### evaluate p1 p2 pck auc dataset with test-flip-augmentation
####################################################################
def evaluate_safety(data_loader, model_pos_eval, device, summary=None, writer=None, key='', tag='', flipaug=''):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    epoch_p1 = AverageMeter()
    epoch_p2 = AverageMeter()
    ...
    for i, temp in enumerate(data_loader):
        ...
        outputs_3d = outputs_3d[:, :, :] - outputs_3d[:, 8:9, :]  # the output is relative to the 8 joint (Neck) 

        # Indexing target joints (Head, Thorax, LShoulder, RShouldeer)
        target_joints = [8, 9, 10, 13]
        targets_3d = targets_3d[:, target_joints, :]
        outputs_3d = outputs_3d[:, target_joints, :]
